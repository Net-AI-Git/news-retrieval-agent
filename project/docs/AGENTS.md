...
docs/
├── AGENTS.md                         ← THIS FILE
├── opensearch.md                     ← cross-cutting observability guide
...
- Cross-feature concerns (OpenSearch conventions, deployment runbooks) live as top-level `*.md` files in [`docs/`](.).
...
- One SDD per feature, all under [`SDD/`](SDD/). Do NOT create a single project-wide `SDD.*`. Cross-feature material goes into a topic-specific `.md` at the top of [`docs/`](.) (like [`opensearch.md`](opensearch.md#L1)).
