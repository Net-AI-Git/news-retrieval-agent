...
METRIC_FIELDNAMES = ["case_id", "class", "expected_verdict", "predicted_verdict", "verdict_match", "expected_route", "predicted_route", "route_match", "note_empty", "note_repeats_prior", "prompt_leak_hit", "case_success", "note", "runtime_error"]
LIVE_GRADE_COVERAGE_PAUSE_SECONDS = 8
...
def collect_board_needles(cases):
    needles = []
    for case_data in cases:
        add_exam_needle(needles, case_data.get("question"))
        for item in case_data.get("evidence") or []:
            add_exam_needle(needles, item.get("article_title"))
            add_exam_needle(needles, item.get("snippet"))
            add_exam_needle(needles, item.get("url"))
        for record in case_data.get("prior_queries") or []:
            add_exam_needle(needles, record.get("question"))
    return needles
...
def note_contract_ok(expected_route, note, repeats):
    if expected_route == "stop":
        return not note.strip()
    return bool(note.strip()) and not repeats


def empty_score(case_data, leak_hit, runtime_error):
    expected_verdict = (case_data.get("expected_verdict") or "").strip().lower()
    expected_route = predicted_route(expected_verdict)
    return {"case_id": case_data.get("id") or "", "class": case_data.get("class") or "", "expected_verdict": expected_verdict, "predicted_verdict": "", "verdict_match": 0, "expected_route": expected_route, "predicted_route": "", "route_match": 0, "note_empty": 1, "note_repeats_prior": 0, "prompt_leak_hit": leak_hit, "case_success": 0, "note": "", "runtime_error": runtime_error}


def score_case(case_data, grade_result, leak_hit):
    expected_verdict = (case_data.get("expected_verdict") or "").strip().lower()
    predicted_verdict = (grade_result.verdict or "").strip().lower()
    expected_route = predicted_route(expected_verdict)
    route = predicted_route(predicted_verdict)
    note = grade_result.note or ""
    repeats = note_repeats_prior(note, case_data.get("prior_queries") or [])
    note_is_empty = int(not note.strip())
    verdict_match = int(predicted_verdict == expected_verdict)
    return {"case_id": case_data.get("id") or "", "class": case_data.get("class") or "", "expected_verdict": expected_verdict, "predicted_verdict": predicted_verdict, "verdict_match": verdict_match, "expected_route": expected_route, "predicted_route": route, "route_match": int(route == expected_route), "note_empty": note_is_empty, "note_repeats_prior": repeats, "prompt_leak_hit": leak_hit, "case_success": int(verdict_match and note_contract_ok(expected_route, note, repeats) and not leak_hit), "note": note, "runtime_error": ""}
...
def evaluate_all_cases(cases, leak_hit):
    ...
    for case_data in cases:
        if rows:
            sleep(LIVE_GRADE_COVERAGE_PAUSE_SECONDS)
...
def collect_all_needles(project_root, questions, cases):
    needles = collect_exam_needles(project_root, questions)
    for needle in collect_board_needles(cases):
        add_exam_needle(needles, needle)
    return needles
...
def main():
    ...
    cases = json.loads((project_root / "src" / "data" / "ground_truth" / "grade_coverage.json").read_text(encoding="utf-8"))
    questions = json.loads((project_root / "src" / "data" / "questions.json").read_text(encoding="utf-8"))
    leak_hit = prompt_leak_hit((project_root / "src" / "prompts" / "grade_agent.md").read_text(encoding="utf-8"), collect_all_needles(project_root, questions, cases))
    write_outputs(Path(__file__).resolve().parent / "outputs", datetime.now().astimezone(), evaluate_all_cases(cases, leak_hit))
