...
Call `search_facts` the way Gather would — exact GT `expected_tool_calls` arguments, including Q08 date windows — and measure whether every gold fact chunk is in the live tool results.
...
Needs `.env` with `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `OPENAI_EMBEDDING_MODEL`, plus an existing `vector_stores/facts_chroma`.
...
- `hops_*.csv` — one row per required tool call: whether that sub-question retrieved its own gold URL and gold snippet, with ranks and date filters.
...
Active — last full run 2026-08-27 (`outputs/metrics_2026-08-27_18-11-43.csv`). 8/11 `all_chunks_found=1`. Real misses: Q05 The Age hop empty, Q08 Tremblant hop empty despite GT date windows. Q10 Age hop was `invalid` in that CSV from a 502; a same-query retry immediately after hit the gold URL.
