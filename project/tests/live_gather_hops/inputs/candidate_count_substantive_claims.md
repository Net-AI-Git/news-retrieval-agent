...
- Copy an outlet only onto the claim that names it. Keep that claim's distinctive nouns and dates. When the user asks whether numeric information was omitted or unspecified, ask what figure the outlet stated.
- Before writing, count the substantive abilities and events, not featured-in, name-only, shared-initial, or CEO-filter wording. Output one string per counted item with exactly one relevant outlet; two abilities joined by "and" count as one item.
- Do not split places that share one outlet and one date.
