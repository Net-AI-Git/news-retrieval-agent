...
Live cases call OpenRouter and the local Chroma stores. They need `.env` with `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `OPENAI_GATHER_AGENT_MODEL`, `OPENAI_GRADE_AGENT_MODEL`, `OPENAI_ANSWER_AGENT_MODEL`, and `OPENAI_RETRIEVE_AGENT_MODEL`, plus existing `output_for_mission/facts_chroma` and `vector_stores/corpus_chroma`.
