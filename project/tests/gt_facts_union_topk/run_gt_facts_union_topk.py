...
def merge_facts_hit(merged, hit, sub_question):
...
def score_question(question_data, ground_truth, top_hits):
    gold_urls = list(dict.fromkeys(item["url"] for item in ground_truth.get("facts") or [] if item.get("url")))
    gold_facts = [item["fact"] for item in ground_truth.get("facts") or [] if item.get("fact")]
    ...
    return {"question_id": question_data["id"], "question": question_data["question"], "answer": ground_truth.get("answer"), "gold_urls": gold_urls, "gold_facts": gold_facts, "top_hits": annotated, **question_metrics(unanswerable, gold_urls, annotated, matched_urls)}
...
def evaluate_one_question(project_root, question_data):
    ...
    for sub_question in ground_truth.get("sub_questions") or []:
        for hit in retrieve_facts_hits(project_root, sub_question):
            merge_facts_hit(merged, hit, sub_question)
...
def summary_lines(rows):
    ...
    return ["## Summary", "", f"- Answerable questions: {len(scored)}", f"- Success@5 (every gold fact URL has a fact in top 5): {success_rate}", f"- Macro Recall@5: {mean_recall}", f"- Macro Precision@5: {mean_precision}", "", "## Questions", ""]
...
def question_markdown(row):
    lines = [f"### {row['question_id']}", "", f"- Question: {row['question']}", f"- GT answer: {row.get('answer')}", f"- Unanswerable: {row['unanswerable']}", f"- Precision@5: {row['precision_at_5']}", f"- Recall@5: {row['recall_at_5']}", f"- Success@5: {row['success_at_5']}", f"- False positives: {row['false_positives']}", f"- Missing gold URLs: {', '.join(row['missing_urls']) or 'None'}", "", "Gold facts:", ""]
...
def build_markdown(rows, timestamp):
    lines = ["# GT Facts Union Top-5", "", f"- Timestamp: {timestamp.isoformat(timespec='seconds')}", f"- Top K: {EVALUATION_TOP_K}", "- Store: facts", ""]
