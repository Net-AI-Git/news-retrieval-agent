...
- A need is covered only by supporting evidence. A named outlet is covered only when an evidence URL or article_title belongs to it, and is searched only when prior_queries.source names it.
