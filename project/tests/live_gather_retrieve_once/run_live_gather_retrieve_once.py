...
def run_experiment_gather(task_data, gather_prompt):
    return ChatOpenAI(model=os.getenv("OPENAI_GATHER_AGENT_MODEL"), api_key=os.getenv("OPENAI_API_KEY"), base_url=os.getenv("OPENAI_BASE_URL"), temperature=0.3, seed=42).with_structured_output(GatherResult).invoke([SystemMessage(gather_prompt), HumanMessage(json.dumps({"question": task_data["question"], "prior_queries": task_data.get("prior_queries") or [], "grade_note": task_data.get("grade_note") or ""}, ensure_ascii=False))])
...
def retrieve_once_node(state, task_data, flow_id, retrieve_prompt):
    ...
    message = ChatOpenAI(model=os.getenv("OPENAI_RETRIEVE_AGENT_MODEL"), api_key=os.getenv("OPENAI_API_KEY"), base_url=os.getenv("OPENAI_BASE_URL"), temperature=0.3, seed=42).bind_tools(build_retrieve_tools(task_data, flow_id)).invoke([SystemMessage(retrieve_prompt), HumanMessage(json.dumps({"question": state["question"], "sub_questions": sub_questions}, ensure_ascii=False))])
    tool_calls = []
