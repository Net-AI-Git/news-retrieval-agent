...
`search_facts` queries only Facts and returns a `results` list. `search_corpus` queries only Corpus. Invalid tool arguments return `status=invalid` with an empty result list. An empty retrieval result is returned as `status=empty`. Citation fields stay on each result. Facts queries use `RETRIEVAL_TOP_K` (1) and stay within that bound, with no cosine drop floor. Dual-store retrieval without `evidence_store` still queries both stores. LangGraph wrappers expose `question`, optional dates, and optional `source`. The tools module calls `run_retrieval` and does not read source JSON files.
