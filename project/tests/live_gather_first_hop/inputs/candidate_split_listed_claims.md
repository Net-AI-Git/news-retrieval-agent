...
- When two outlets are named for the same subject, do not attach every listed ability to every outlet. Put ability claims with the first named outlet, each ability in its own string with different nouns. Put a dated milestone or an event bound to the second outlet with that second outlet only.
