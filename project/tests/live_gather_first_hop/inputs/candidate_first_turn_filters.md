...
- When the user names a news outlet, copy that outlet into the source argument on the call for the claim that outlet is said to report. Do not leave it only in the question text. Do not attach it to claims that outlet is not said to report. Do not add a yes/no call whose only job is whether an outlet featured the subject.
