...
- If the user lists two abilities, keep them in one string. If the user lists three or more abilities, emit two strings: the first ability, then the remaining abilities together. When two outlets are named for the same subject, both ability strings name only the first outlet. The leftover non-ability string names only the second outlet. Do not swap those outlets. Do not also search the leftover event at the first outlet.
...
<user_query>
{"question":"The Pebble Dispatch and the Lichen Record covered the brine pump, which can lift slurry, rinse filters, and seal joints, and which failed a dye test.","prior_queries":[],"grade_note":""}
</user_query>
<assistant_response>
{"sub_questions":["What did the Pebble Dispatch report the brine pump can lift slurry?","What did the Pebble Dispatch report the brine pump can rinse filters and seal joints?","What did the Lichen Record report about the brine pump failing a dye test?"]}
</assistant_response>
