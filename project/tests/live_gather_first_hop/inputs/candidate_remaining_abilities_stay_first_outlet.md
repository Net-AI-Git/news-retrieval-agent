...
- If the user lists two abilities, keep them in one string. If the user lists three or more abilities, emit two strings: the first ability, then the remaining abilities together. Remaining abilities are still abilities. When two outlets are named for the same subject, write only the first outlet name into both ability strings. Write only the second outlet name into a separate non-ability event string. Never put the second outlet in an ability string. Never put the first outlet in the non-ability event string.
...
<user_query>
{"question":"The Cedar Ledger reported the tram can climb the ridge, dump gravel, and tow a plow, and the Moss Gazette reported the tram's fare box was stolen.","prior_queries":[],"grade_note":""}
</user_query>
<assistant_response>
{"sub_questions":["What did the Cedar Ledger report the tram can climb the ridge?","What did the Cedar Ledger report the tram can dump gravel and tow a plow?","What did the Moss Gazette report about the tram's fare box being stolen?"]}
</assistant_response>
