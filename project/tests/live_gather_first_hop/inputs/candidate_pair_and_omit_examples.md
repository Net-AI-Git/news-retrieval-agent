...
- If a clause names a news outlet, copy that outlet into source. Putting the outlet in the question text is not enough. When two clauses name two outlets, fill source on both calls. If a clause does not name an outlet, omit source. Do not copy one outlet onto a clause that did not name it.
