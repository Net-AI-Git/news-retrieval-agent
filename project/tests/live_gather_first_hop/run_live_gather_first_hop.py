...
from src.conts import CORPUS_CHROMA_PATH, FACTS_CHROMA_PATH, GROUNDED_ANSWERING_RECURSION_LIMIT
from src.orchestration.grounded_answering_workflow import GroundedAnsweringState, gather_node, retrieve_node, route_after_gather, route_after_retrieve, tools_node
...
def build_first_hop_graph(task_data, flow_id):
...
def run_first_hop(question):
    task_data = {"question": question, "facts_chroma_path": FACTS_CHROMA_PATH, "corpus_chroma_path": CORPUS_CHROMA_PATH}
    graph_state = build_first_hop_graph(task_data, str(uuid4())).invoke({"question": question, "messages": [HumanMessage(question)], "evidence": [], "prior_queries": [], "sub_questions": [], "gather_count": 0, "tool_count": 0, "grade_verdict": None, "grade_note": None, "answer_result": None}, {"recursion_limit": GROUNDED_ANSWERING_RECURSION_LIMIT})
...
def evaluate_question(project_root, question_data, leak_hit):
    ...
    try:
        task_data, graph_state = run_first_hop(question_data["question"])
...
def main():
    ...
    leak_hit = prompt_leak_hit((project_root / "src" / "prompts" / "gather_agent.md").read_text(encoding="utf-8") + (project_root / "src" / "prompts" / "retrieve_agent.md").read_text(encoding="utf-8"), collect_exam_needles(project_root, questions))
