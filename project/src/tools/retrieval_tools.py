...
from ..conts import RETRIEVAL_EVIDENCE_STORE_CORPUS, RETRIEVAL_EVIDENCE_STORE_FACTS, RETRIEVAL_TOOL_STATUS_INVALID
...
class RetrievalTools:
    ...
    def search_facts(self, question: str, published_from: Optional[str] = None, published_to: Optional[str] = None) -> dict:
        """Search curated one-sentence facts. Call this first when a short factual answer may exist. Optional published_from and published_to are inclusive ISO-8601 timestamps that keep only facts whose article date is in range. Returns status, the query, and a bounded citation-ready result list."""
        try:
            payload = SearchEvidenceInput.model_validate({"question": question, "published_from": published_from, "published_to": published_to})
