...
- **Shared infra** → [`redis_repository.py`](redis_repository.py:1), [`opensearch_repository.py`](opensearch_repository.py:1). Optional vendor-client stubs shipped as reusable examples: [`pinecone_repository.py`](pinecone_repository.py:1), [`embeddings_repository.py`](embeddings_repository.py:1) — delete any your service does not use.
...
- Every method accepts `task_data` and `flow_id`. Logs the full `task_data` via [`opensearch_repository.py`](opensearch_repository.py:1) for batch traceability.
...
- Imports: `os`, `dotenv`, `sqlalchemy` (`create_engine`, `text`, `Engine`), `OpenSearchRepository`.
...
- Imports: `os`, `ssl`, `tempfile`, `time`, `requests`, `urllib3`, `cryptography`, `dotenv`, `OpenSearchRepository`.
...
- [`.Codex/rules/04-error-and-logging.md`](../../../.Codex/rules/04-error-and-logging.md:1) — single try/except + OpenTelemetry event shape.
