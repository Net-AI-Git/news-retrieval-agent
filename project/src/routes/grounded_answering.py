...
def grounded_answering(body: Request):
    flow_id = str(uuid4())
    return Response(content=json.dumps(run_grounded_answering({"question": body.content, "facts_chroma_path": FACTS_CHROMA_PATH, "corpus_chroma_path": CORPUS_CHROMA_PATH}, flow_id), ensure_ascii=False))
