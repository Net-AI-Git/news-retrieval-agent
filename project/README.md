...
Chat and embeddings stay on OpenRouter. The chat model is `openai/gpt-4o-mini` via `OPENAI_MODEL`, separate from `OPENAI_EMBEDDING_MODEL`. Gather, retrieve, Grade, and Answer construct `ChatOpenAI` from env as runtime, not through a GPT repository.
