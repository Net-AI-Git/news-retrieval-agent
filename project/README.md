...
Chat and embeddings stay on OpenRouter. Gather reads `openai/gpt-4.1-mini` from `OPENAI_GATHER_MODEL`. Retrieve, Grade, and Answer read `openai/gpt-4o-mini` from `OPENAI_MODEL`. Embeddings stay on `OPENAI_EMBEDDING_MODEL`. Each agent constructs `ChatOpenAI` from env as runtime, not through a GPT repository.
...
3. **Stronger model vs split.** Cross-claim leakage is structural. Retrieve, Grade, and Answer stay on `OPENAI_MODEL` (`gpt-4o-mini`). Gather uses `OPENAI_GATHER_MODEL` (`gpt-4.1-mini`) so a stronger Gather slug does not change the other agents.
