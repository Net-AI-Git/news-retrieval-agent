...
The baseline still performs no lexical fusion, per-article cap or answer generation inside retrieval. Gather still decomposes the question and calls `search_facts` per hop. Each hop returns only the top Facts chunk. Temporal comparison remains possible because every result preserves `published_at`.
