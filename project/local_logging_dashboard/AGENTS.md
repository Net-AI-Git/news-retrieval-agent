# local_logging_dashboard — Agent Guide

## Purpose
Generate a self-contained Plotly HTML dashboard from events written by `LocalLoggingRepository`. The microservice never reads this directory at runtime.

## Contains
- `build_dashboard.py` — reads the `local_logs` SQLite table and generates the dashboard.
- `README.md` — one-command usage and panel inventory.
- `dashboard.html` — generated locally and never committed.

## Rules
- Every dashboard field must map to `time` or one of the six fields in [`../docs/local_logging.md`](../docs/local_logging.md).
...
## See Also
- [`../docs/local_logging.md`](../docs/local_logging.md)
- [`../src/repositories/local_logging_repository.py`](../src/repositories/local_logging_repository.py)
