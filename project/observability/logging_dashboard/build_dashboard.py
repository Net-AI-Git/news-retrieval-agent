...
def load_dashboard_data():
    ...
    try:
        totals = connection.execute("SELECT count(*) AS total, COALESCE(sum(CASE WHEN level = 'ERROR' THEN 1 ELSE 0 END), 0) AS errors FROM logs").fetchone()
        return {"total": totals[0], "errors": totals[1], "statuses": connection.execute("SELECT status, count(*) AS events FROM logs GROUP BY status ORDER BY events DESC").fetchall(), "error_processes": connection.execute("SELECT process, count(*) AS errors FROM logs WHERE level = 'ERROR' GROUP BY process ORDER BY errors DESC LIMIT 10").fetchall(), "timeline": connection.execute("SELECT substr(time, 1, 16) AS minute, count(*) AS events FROM logs GROUP BY minute ORDER BY minute").fetchall(), "recent_errors": connection.execute("SELECT time, process, flow_id, content FROM logs WHERE level = 'ERROR' ORDER BY time DESC LIMIT 20").fetchall()}
    finally:
        connection.close()
